...
Use CAI tools to scan for idle static IPs and unattached disks, and spend analysis precomputations to scan for storage and replica waste.
...
To check GCS and Secret Manager waste, you MUST check the blackboard for the 'gcs_secret_waste' key. If it is NOT present in the session state blackboard, you MUST call the `get_precomputed_spend_analysis` tool to run the precomputation and automatically cache GCS and Secret Manager waste on the blackboard. Do NOT write or execute raw BigQuery SQL queries to scan for storage and secret waste.
...
Whenever you need to run CAI zombie resource scans (e.g. scanning for both static IPs and unattached disks), you MUST call these tools in parallel in a single turn. Do NOT query them sequentially, as this dramatically increases execution latency.
