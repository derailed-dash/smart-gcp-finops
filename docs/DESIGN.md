...
## 1. Overview & Creative Vision: "The Neon Core"

We are using **Emerald Cyber Aesthetics** - a high-tech, high-contrast dark-mode styling system using pure CSS custom properties, obsidian cards, glowing neon borders (`#00F59B0F` default), and glowing cyber-teal charts, completely avoiding flat border strokes or muddy shadows.
...
A split-screen canvas layout with a persistent 35% Chat Copilot on the left and a dynamic workspace canvas on the right. This balances natural language conversation with data-rich observability (large tables and charts), enabling the agent to "project" rich UI components to the canvas via the A2UI protocol. |
...
A glassmorphic vertical scanning _visual sync overlay_ triggers automatically when the JSON A2UI payload is streaming. This solves the UX "frozen visual state" by clearly indicating to the user that the canvas data is actively synchronising.
