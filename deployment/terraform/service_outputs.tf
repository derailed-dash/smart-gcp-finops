...
output "cloud_run_domain_mappings" {
  description = "Cloud Run domain mapping records for DNS configuration in IONOS"
  value = {
    for k, v in google_cloud_run_domain_mapping.app_domain_mapping : k => {
      name    = v.name
      records = v.status[0].resource_records
    }
  }
}
